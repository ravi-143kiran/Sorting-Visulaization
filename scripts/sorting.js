...
function generateBars(noBar=60){
     // Clear the content of the element with id "bar"
    document.getElementById("bar").innerHTML='';
    let bars=[];
      // Generate "noBar" random values and push them into the "bars" array
    for(let i=0;i<noBar;i++){
        bars.push(Math.floor(Math.random()*400+1)) //it generates a random integer between 1 and 400 (inclusive)
    }
...
document.getElementById("newarr").addEventListener("click",function(){       // Add an event listener to the element with ID "newarr" when it's clicked.
...
//  This function is used to introduce a delay or pause in the execution of asynchronous code
